...
### Evaluate OM data in the beginning of the experiment
 plotOMBefore=ldply(grouped.OMdataBefore, data.frame)          #move the dataframes for each group into a single dataframe
 plotOMBefore=plotOMBefore[,c("time","means","sd","group")]

 #plot averaged OM traces

  print(ggplot(plotOMBefore, aes(x=time/1000, y=means, group = group)) +
        theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_line( size=.1, color="grey"))+
        geom_rect(aes(xmin = mean(plotOMBefore$time/1000),xmax = Inf ,ymin = -Inf, ymax = Inf),fill=("grey"), alpha = 0.01)+
        geom_ribbon(aes(ymin=means-sd, ymax=means+sd, fill = group), alpha=0.5) +
        geom_hline(yintercept = 0, color="black") +
        scale_fill_manual(values = boxcolors) +
        geom_line(aes(colour = group), size = 1) + 
...
### Evaluate OM data at the end of the experiment
 plotOMAfter=ldply(grouped.OMdataAfter, data.frame)            #move the dataframes for each group into a single dataframe
 plotOMAfter=plotOMAfter[,c("time","means","sd","group")]
 
 #plot averaged OM traces

  print(ggplot(plotOMAfter, aes(x=time/1000, y=means, group = group)) +
        theme(panel.grid.major.x = element_blank(),panel.grid.major.y = element_line( size=.1, color="grey"))+
        geom_rect(aes(xmin = mean(plotOMAfter$time/1000),xmax = Inf ,ymin = -Inf, ymax = Inf),fill=("grey"), alpha = 0.01)+
        geom_ribbon(aes(ymin=means-sd, ymax=means+sd, fill = group), alpha=0.5) +
        geom_hline(yintercept = 0, color="black") +
        scale_fill_manual(values = boxcolors) +
        geom_line(aes(colour = group), size = 1) + 
...
samplesizes = as.numeric(table(plotOMparamsBefore$group)) #determine sample sizes in each group
...
if(two)
  {
        for (v in 1:3) {
           cat(paste("\n\n## ", OMtitles[v], " at start of experiment\n\n", sep = ""))
            utest = signif(wilcox.test(plotOMparamsBefore[[OMvariables[v]]] ~ plotOMparamsBefore$group)$p.value, 3) #compare the two groups with a U-test and collect p-value
            w.statistic = signif(wilcox.test(plotOMparamsBefore[[OMvariables[v]]] ~ plotOMparamsBefore$group)$statistic, 3)
            #compute effect size Cohen's D
            cohend = signif(cohen.d(plotOMparamsBefore[[OMvariables[v]]] ~ plotOMparamsBefore$group)$estimate, 3)
            #calculate statistical power
            alt = project.data[["statistics"]][["two.groups"]][["power"]]
            power=signif(pwr.t2n.test(n1 = samplesizes[1], n2= samplesizes[2], d = cohend, alternative = alt, sig.level = signif[1])$power, 3)
            #calculate Bayes Factor
            bayesF=extractBF(ttestBF(plotOMparamsBefore[[OMvariables[v]]][plotOMparamsBefore$group==groupnames[1]], plotOMparamsBefore[[OMvariables[v]]][plotOMparamsBefore$group==groupnames[2]]))
            #calculate FPR for priors set in project file#
            #run first prior  
              prior=priorval[1]
              out=calc.FPR(samplesizes,utest,prior,abs(cohend))  #output=c(FPR,x0,y0,x1,y1)
        ...
        # plot two PIs with asterisks
          plots.2test<-list(ggplot(plotOMparamsBefore, aes(group, plotOMparamsBefore[[OMvariables[v]]])) +
              geom_boxplot(fill = boxcolors, notch = TRUE, outlier.color=NA, width=0.8, size=0.6) +
              geom_jitter(data = plotOMparamsBefore, aes(group, plotOMparamsBefore[[OMvariables[v]]]), position=position_jitter(0.3), shape=21, size=3, colour="black", fill="grey50", alpha=0.4) +
              ggtitle(paste("U-Test, p=", utest)) +
              theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black", fill=NA)) +
...
samplesizes = as.numeric(table(plotOMparamsAfter$group)) #determine sample sizes in each group
...
if(two)
  {
        for (v in 1:3) {
           cat(paste("\n\n## ", OMtitles[v], " at end of experiment\n\n", sep = ""))
            utest = signif(wilcox.test(plotOMparamsAfter[[OMvariables[v]]] ~ plotOMparamsAfter$group)$p.value, 3) #compare the two groups with a U-test and collect p-value
            w.statistic = signif(wilcox.test(plotOMparamsAfter[[OMvariables[v]]] ~ plotOMparamsAfter$group)$statistic, 3)
            #compute effect size Cohen's D
            cohend = signif(cohen.d(plotOMparamsAfter[[OMvariables[v]]] ~ plotOMparamsAfter$group)$estimate, 3)
            #calculate statistical power
            alt = project.data[["statistics"]][["two.groups"]][["power"]]
            power=signif(pwr.t2n.test(n1 = samplesizes[1], n2= samplesizes[2], d = cohend, alternative = alt, sig.level = signif[1])$power, 3)
            #calculate Bayes Factor
            bayesF=extractBF(ttestBF(plotOMparamsAfter[[OMvariables[v]]][plotOMparamsAfter$group==groupnames[1]], plotOMparamsAfter[[OMvariables[v]]][plotOMparamsAfter$group==groupnames[2]]))
            #calculate FPR for priors set in project file#
            #run first prior  
              prior=priorval[1]
              out=calc.FPR(samplesizes,utest,prior,abs(cohend))  #output=c(FPR,x0,y0,x1,y1)
        ...
        # plot two PIs with asterisks
          plots.2test<-list(ggplot(plotOMparamsAfter, aes(group, plotOMparamsAfter[[OMvariables[v]]])) +
              geom_boxplot(fill = boxcolors, notch = TRUE, outlier.color=NA, width=0.8, size=0.6) +
              geom_jitter(data = plotOMparamsAfter, aes(group, plotOMparamsAfter[[OMvariables[v]]]), position=position_jitter(0.3), shape=21, size=3, colour="black", fill="grey50", alpha=0.4) +
              ggtitle(paste("U-Test, p=", utest)) +
              theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black", fill=NA)) +
