...
if(NofGroups==3)
{
  ...
  for (v in 1:3) {
    ...
    plots.2test<-ggplot(plotOMparamsBefore, aes(group, plotOMparamsBefore[[OMvariables[v]]])) +
      geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
      geom_boxplot(fill = boxcolors, notch = TRUE, outlier.color=NA, width=0.8, size=0.6) +
      geom_jitter(data=PIstatCombined, aes(group, plotOMparamsBefore[[OMvariables[v]]]), position=position_jitter(0.3), shape=21, size=3, colour="black", fill="grey50", alpha=0.4)
      ...
      theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.text.y = element_text(size=18))+ 
      ylab(paste("PI", learningscore, " [rel. units]", sep = ""))+ xlab("Groups")+ theme(aspect.ratio=3/NofGroups)+
      geom_signif(comparisons = list(c(colnames(plotOMparamsBefore[[OMvariables[v]]][2]), colnames(plotOMparamsBefore[[OMvariables[v]]][3]))), 
                  map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                  textsize=5, vjust=0.1) +
      geom_signif(comparisons = list(c(colnames(plotOMparamsBefore[[OMvariables[v]]][1]), colnames(plotOMparamsBefore[[OMvariables[v]]][3]))), 
                  map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
...
if(NofGroups==3){
  ...
  for (v in 1:3) {
    ...
    # plot two PIs with asterisks
      ...
      geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
      geom_boxplot(fill = boxcolors, notch = TRUE, outlier.color=NA, width=0.8, size=0.6) +
      geom_jitter(data=PIstatCombined, aes(group, plotOMparamsAfter[[OMvariables[v]]]), position=position_jitter(0.3), shape=21, size=3, colour="black", fill="grey50", alpha=0.4)
      scale_y_continuous(breaks = seq(-1, 1, .2)) +
      theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(),
                                          panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black",
                                                                      fill=NA), legend.position = "top", 
      ...
      theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.text.y = element_text(size=18))+ 
      ylab(paste("PI", learningscore, " [rel. units]", sep = ""))+ xlab("Groups")+ theme(aspect.ratio=3/NofGroups)+
      geom_signif(comparisons = list(c(colnames(plotOMparamsAfter[[OMvariables[v]]][2]), colnames(plotOMparamsBefore[[OMvariables[v]]][3]))), 
                  map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                  textsize=5, vjust=0.1) +
      geom_signif(comparisons = list(c(colnames(plotOMparamsAfter[[OMvariables[v]]][1]), colnames(plotOMparamsAfter[[OMvariables[v]]][3]))), 
                  map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                  textsize=5, vjust=0.1, y_position =1.2) +
      ylim(c(-1, 1.3)) +
      stat_n_text()
