...
exp_groups[1, ] <- paste('<a href="http://flybase.org/reports/',groupids,'">',groupids,'</a>', sep = '')
...
```{r OMparams, eval=om, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 9.5, comment=NA, results='asis'}
...
OMvariables = head(names(plotOMparams),-2)
...
if(NofGroups==1)
  {
        for (v in 1:3) {
           cat(paste("\n\n## ", OMtitles[v], "\n\n", sep = ""))
           print(ggplot(plotOMparams, aes(name, plotOMparams[[OMvariables[v]]])) +
...
if(NofGroups==3){
OMtitles = c("Optomotor Magnitude", "Rate Constant", "Asymmetry Index")
threegroup(grouped.OMparams, plotOMparams)
}
...
if(NofGroups==1)
  {
        for (v in 1:3) {
           cat(paste("\n\n## ", OMtitles[v], " at start of experiment\n\n", sep = ""))
           print(ggplot(plotOMparamsBefore, aes(name, plotOMparamsBefore[[OMvariables[v]]])) +
              geom_boxplot(fill = boxcolors, notch = FALSE, outlier.color=NA, width=0.8, size=0.6) +
              geom_jitter(data = plotOMparamsBefore, aes(name, plotOMparamsBefore[[OMvariables[v]]]), position=position_jitter(0.3), shape=21, size=3, colour="black", fill="grey50", alpha=0.4) +
              theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black", fill=NA)) +
              theme(axis.text.y = element_text(size=18))+ ylab(paste(OMtitles[v], " [rel. units]", sep = ""))+ xlab("Groups")+ theme(aspect.ratio=3/NofGroups)+
              samplesizes.annotate(boxes, samplesizes))
        }
...
OMtitles = c("Optomotor Magnitude", "Rate Constant", "Asymmetry Index")
threegroup(grouped.OMparamsBefore, plotOMparamsBefore)}
...
OMvariables = head(names(plotOMparamsAfter),-2)
OMtitles = c("Optomotor Magnitude", "Rate Constant", "Asymmetry Index")
...
OMtitles = c("Optomotor Magnitude", "Rate Constant", "Asymmetry Index")
threegroup(grouped.OMparamsAfter, plotOMparamsAfter)}
...
# plot PI box plot test against zero
  plots.singles<-list(ggplot(PIstatCombined, aes(name, PIs)) +
    geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
    geom_boxplot(fill = boxcolors, notch = FALSE, outlier.color=NA, width=0.8, size=0.6) +
    ggtitle("Wilcoxon") +
    geom_jitter(data=PIstatCombined, aes(name, PIs, fill=category), position=position_jitter(0.3), shape=21, size=3, alpha=0.5) +
    scale_y_continuous(breaks = seq(-1, 1, .2)) +
...
```{r Utest, eval=(twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 10, comment=NA, results='asis'}
cat("\n## Statistical tests between two groups\n")
    ...
    if(fligner.test(PIstatCombined$PIs,PIstatCombined$name)$p.value<0.05){hv=FALSE}else{hv=TRUE}       #test for homogeneity of variance and set hv TRUE/FALSE
    if(equalN & hv){hedges=FALSE}else{hedges=TRUE}                      #test if we need to apply Hedge's correction
    if(hedges){
      cohend = signif(cohen.d(PIstatCombined$PIs, PIstatCombined$name, hedges.correction = hedges)$estimate, 3) #use effsize package for Hedge's correction
    }else{
...
# plot two PIs with asterisks
  plots.2test<-list(ggplot(PIstatCombined, aes(name, PIs)) +
      geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
...
```{r estimationstats, eval=(twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
cat("\n## Estimation statistics\n")
unpaired_median_diff <- dabest(PIstatCombined, name, PIs, idx = groupnames, paired = FALSE, func = median) #create estimation results list
print(plot(unpaired_median_diff)) #plot results
```

```{r splitviolins, eval=(twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
cat("\n## Split Violin Plot\n")

PIstatCombined$learningscore=project.data$statistics$`learning-score`$title #add x-axis value for split violin plots
print(ggplot(PIstatCombined, aes(learningscore, PIs, fill = name)) + scale_fill_manual(values=boxcolors) + geom_split_violin()) #plot spit violin plot

```

```{r UtestG3, eval=(threegroupstats & NofGroups==3), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 12, comment=NA, results='asis'}
cat("\n## Statistical tests between three groups\n")
#creating a list with PIs and condition in on list

data <- list()
PItype = list()
OMtitles = paste0("PI ", learningscore)
for (x in 1:NofGroups){
...
OMvariables = "PIs"
threegroup(PItype, PIstatCombined)

```

```{r Utestpooled, eval=(twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 12, comment=NA, results='asis'}
...
reactable(PooledGroups,
  ...
  fullWidth = FALSE)
    ...
    #compute effect size Cohen's D
    if(samplesizes[1]==samplesizes[2]){equalN=TRUE}else{equalN=FALSE}   #deterine if the samplesizes are identical
    if(fligner.test(PIstatPooled$PIs,PIstatPooled$name)$p.value<0.05){hv=FALSE}else{hv=TRUE}       #test for homogeneity of variance and set hv TRUE/FALSE
    if(equalN & hv){hedges=FALSE}else{hedges=TRUE}                      #test if we need to apply Hedge's correction
    if(hedges){
      cohend = signif(cohen.d(PIstatPooled$PIs, PIstatPooled$name, hedges.correction = hedges)$estimate, 3) #use effsize package for Hedge's correction
    }else{
...
# plot two PIs with asterisks
  plots.2test<-list(ggplot(PIstatPooled, aes(name, PIs)) +
      geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
      geom_boxplot(fill = boxcolors[1:2], notch = TRUE, outlier.color=NA, width=0.8, size=0.6) +
      geom_jitter(data=PIstatPooled, aes(name, PIs, fill=category), position=position_jitter(0.3), shape=21, size=3) +
      ggtitle(paste("U-Test, p=", utest)) +
      scale_y_continuous(breaks = seq(-1, 1, .2)) +
...
```{r estimationstatspooled, eval=(twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
 ...
 unpaired_median_diff <- dabest(PIstatPooled, name, PIs, idx = groupnames, paired = FALSE, func = median) #create estimation results list
 print(plot(unpaired_median_diff)) #plot results

```

```{r splitviolinspooled, eval=(twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
...
print(ggplot(PIstatPooled, aes(learningscore, PIs, fill = name)) + scale_fill_manual(values=boxcolors) + geom_split_violin()) #plot spit violin plot
