...
```{r FlyHistogram, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 12, fig.width = 8, comment=NA, results='asis'}    
...
## alternative Fly Behavior Histograms

```{r altFlyHistogram, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 12, fig.width = 8, comment=NA, results='asis'}    
  ...
  for(x in 1:NofGroups)
  {
    ...
    for (i in 1:NofPeriods){
      ...
      hist(x = df$fly, breaks = 100,xlim = maxfly, xlab="Temperature in degrees Fahrenheit", border= sequence$histocolor[i],  col = sequence$histocolor[i], main=paste("Period ", i))
      abline(v = 0, col="black", lwd=2, lty=2)
    }
    maxfly = c(-round_any(max(abs(range(grouphistdata[[x]]$fly))), 100, f=ceiling),round_any(max(abs(range(grouphistdata[[x]]$fly))), 100, f=ceiling))
    hist(grouphistdata[[x]]$fly, breaks = 100,xlim = maxfly, xlab=paste(FlyBehavior, "[arb units]"), main=paste("Pooled Behavior Histogram", groupnames[[x]]))
  }  
```
...
`r if (om | beforeafter) '# Optomotor evaluations normalized'`
`r if (om) '## Optomotor traces (right/left) normalized'`
```{r OptomotorNormalized, eval=om, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 10, comment=NA, results='asis'}

plotOMtracesMean(grouped.OMdatanorm) #call function to plot OM traces averaged over all flies
```
...
`r if (om | beforeafter) '## Optomotor Parameter Analysis normalized'`

```{r OMparamsnorm, eval=om, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}

####### Do not separate optomotor parameters at all ##########
plotOMparams=ldply(grouped.OMparamsnorm, data.frame)    #move the dataframes from each group into a single dataframe
OMvariables = head(names(plotOMparams),-1)
OMtitles = c("Optomotor Magnitude", "Rate Constant", "Asymmetry Index")
...
```{r dwellingalternative, eval=Dwell, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 5, comment=NA, results='asis'}
cat("\n# Dwelling Times\n")
dwellmeans = list()

for(x in 1:NofGroups){
  grouped.dwell[[x]]$punished = -grouped.dwell[[x]]$punished #invert the punished values to make the bars go downwards
  dwellmeans[[x]] = reshape2::melt(grouped.dwell[[x]])
  dwellmeans[[x]]$flyname = rep(flynames[,x][!is.na(flynames[,x])])
}

names(dwellmeans) = groupnames
dwellmeansdata = bind_rows(dwellmeans, .id = "group")
colnames(dwellmeansdata)=c("group","period","value","Outcome","flyname")


dwellplot = ggplot(dwellmeansdata, aes(as.factor(period), value, fill = Outcome)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 0, 
           alpha = .3)+
  stat_summary(geom = "bar", fun = mean, col="black", width = 0.7)+
  stat_summary(geom = "errorbar", fun.data = mean_se, width = 0.3)+
  geom_jitter_interactive(tooltip = dwellmeansdata$flyname, data_id = dwellmeansdata$flyname , col = "grey", pch=21, size=1.0, width = 0.25)+
  ylim(dwellrange)+
...
girafe( ggobj = plot_grid(dwellplot),
        options = list(opts_selection(type = "single",css = "fill:orange;stroke:gray;r:2pt;", only_shiny = FALSE)), width_svg = 4, height_svg = 4)
```
...
for(x in 1:NofGroups)
  {
    ...
    # plot graph
     PIplots[[x]] <- ggplot(PIprofile, aes(x=period, y=PIs)) + 
              geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
              stat_summary(geom = "bar", fun = mean, position = "dodge", fill=as.vector(na.omit(sequence$color)), colour="black", width=1) +
...
```{r BoxWithNotchjitter, eval=PIs, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 15, fig.width = 10, comment=NA, results='asis'}
...
#grid.arrange(grobs = PIplots, nrow=NofGroups)
girafe( ggobj = plot_grid(plotlist=PIplots, ncol = 1),options = list(opts_selection(type = "single",css = "fill:orange;stroke:gray;r:4pt;", only_shiny = FALSE)), width_svg = 3.5+(length(unique(PIprofile$period))), height_svg = 4*NofGroups)
```
...
#grid.arrange(grobs = PIplots, nrow=NofGroups)
girafe( ggobj = plot_grid(plotlist=PIplots, ncol = 1),options = list(opts_selection(type = "single",css = "fill:orange;stroke:gray;r:2pt;", only_shiny = FALSE)), width_svg = 3.5+(length(unique(PIprofile$period))), height_svg = 4*NofGroups)
```
...
```{r test, eval=(PIs & twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
cat("\n## Split Violin Plot for pooled groups\n")
