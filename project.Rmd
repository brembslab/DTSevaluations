  ...
  for (v in 1:3) {
    ...
    plots.2test<-ggplot(plotOMparamsBefore, aes(group, plotOMparamsBefore[[OMvariables[v]]])) +
      ...
      theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(),
                    ...
                    theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.text.y = element_text(size=18))+ 
                          ylab(paste(OMtitles[v], " [rel. units]", sep = ""))+ xlab("Groups")+ theme(aspect.ratio=3/NofGroups) +
                    geom_signif(comparisons = list(c(colnames(grouped.OMparamsBefore[[OMvariables[v]]][2]), colnames(grouped.OMparamsBefore[[OMvariables[v]]][3]))), 
                                map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                                textsize=5, vjust=0.1) +
                    geom_signif(comparisons = list(c(colnames(grouped.OMparamsBefore[[OMvariables[v]]][1]), colnames(grouped.OMparamsBefore[[OMvariables[v]]][3]))), 
                                map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                                textsize=5, vjust=0.1, y_position =1.2) +
      stat_n_text()
  ...
  for (v in 1:3) {
    ...
    plots.2test<-ggplot(plotOMparamsAfter, aes(group, plotOMparamsAfter[[OMvariables[v]]])) +
      ...
      theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(),
                    ...
                    theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.text.y = element_text(size=18))+ 
                          ylab(paste(OMtitles[v], " [rel. units]", sep = ""))+ xlab("Groups")+ theme(aspect.ratio=3/NofGroups) +
                    geom_signif(comparisons = list(c(colnames(grouped.OMparamsAfter[[OMvariables[v]]][2]), colnames(grouped.OMparamsAfter[[OMvariables[v]]][3]))), 
                                map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                                textsize=5, vjust=0.1) +
                    geom_signif(comparisons = list(c(colnames(grouped.OMparamsAfter[[OMvariables[v]]][1]), colnames(grouped.OMparamsAfter[[OMvariables[v]]][3]))), 
                                map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                                textsize=5, vjust=0.1, y_position =1.2) +
      stat_n_text()
