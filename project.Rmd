...
if(NofGroups==3)
{
  ...
  for (t in 1:NofGroups) {
    if(grepl("test", grouped.OMparamsBefore[[t]]$desc) | grepl("exp", grouped.OMparamsBefore[[t]]$desc) | grepl("experimental",grouped.OMparamsBefore[[t]]$desc)==TRUE){
  ...
  for (v in 1:3) {
    ...
    bayesF2=extractBF(ttestBF(Control[[1]][[v]], Control[[2]][[v]]))
    #calculate FPR for priors set in project file#
    #run first prior  
    prior=priorval[1]
...
if(NofGroups==3){
  ...
  Exp=list()
  for (t in 1:NofGroups) {
    if(grepl("test", grouped.OMparamsAfter[[t]]$desc) | grepl("exp", grouped.OMparamsAfter[[t]]$desc) |      grepl("experimental",grouped.OMparamsAfter[[t]]$desc)==TRUE){
...
Exp=list()
for (t in 1:NofGroups) {
  if(grepl("test", PItype[[t]]$desc) | grepl("exp", PItype[[t]]$desc) | grepl("experimental", PItype[[t]]$desc)==TRUE){
...
PIname<-c(unique(Exp$name), unique(Control[[1]]$name),unique(Control[[2]]$name)) #getting names for labeling
#drop second column
