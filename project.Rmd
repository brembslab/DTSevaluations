...
```{r Mean dwelling, eval=PIs, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = len, fig.width = 8, comment=NA, results='asis'}
meanplots = list() #create a list to store all the plots

for(x in 1:NofGroups){
  means[[x]] = do.call(rbind, meandata[[x]]) #load each group of data into a dataframe
  means[[x]]$value[means[[x]]$punishment == 'punished'] <- -means[[x]]$value[means[[x]]$punishment == 'punished'] #To get the data around the zero axis, I am inverting the punished data. I am aware that this is not an accurate data representation, but it works. The axis is then set to only show absolute values

  means = as.data.frame(means[[x]]) #transform into a dataframe

  meanplots[[x]] <- ggplot(means, aes(periodtest, value, fill = punishment)) +
    ...
    geom_jitter(aes(colour = punishment))+
