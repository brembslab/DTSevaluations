...
```{r Grapghparameters, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE, error=FALSE}
if(lengths(project.data["resources"]) == 1){
len = as.integer(5)
} else {
len = as.integer(8)
}
aspect_ratio = nrow(drop_na(sequence))
#aspect_ratio = nrow(sequence %>% filter( contingency != "left_torque" | contingency == "right_torque" | contingency == "color" | contingency == "sw" | contingency == "fs" | contingency == "yt"))
...
for(x in 1:NofGroups)
  {
    ...
    PIprofile <- PIprofile[colSums(!is.na(PIprofile)) > 0] #remove empty columns
     ...
     PIplots[[x]] <- ggplot(error, aes(x=period, y=mean)) + 
              ...
              ylab("PI [rel. units]") + 
              theme(aspect.ratio=4/aspect_ratio)
  }
