...
`r if (om) '# Optomotor Analysis'`

```{r Optomotor, eval=om, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 12, fig.width = 8, comment=NA, results='asis'}
 plotOM <- as.data.frame(do.call(cbind, lapply(grouped.OMdata, "[", , "time"))) #create dataframe from which to plot average optomotor traces for group
 names(plotOM)[1] = "time" #label the first column as time
 
 #extract means, sd and group columns from list by groups
  plotOM$means <- do.call(cbind, lapply(grouped.OMdata, "[", , "means"))
  plotOM$sd <- do.call(cbind, lapply(grouped.OMdata, "[", , "sd"))
  plotOM$group <- do.call(cbind, lapply(grouped.OMdata, "[", , "group"))
