...
**Project file (YAML):** `r paste('<a href="../', basename(project.file), '">', project.data$title, '</a>', sep = "")`  
...
```{r Grapghdim, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = len, fig.width = 6}
...
```{r PIbar, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = len, fig.width = 8}
...
for(x in 1:NofGroups)
  {
    ...
    PIprofile <- PIprofile[colSums(!is.na(PIprofile)) > 0] #remove empty columns
     ...
     PIplots[[x]] <- ggplot(error, aes(x=period, y=mean)) + 
              ...
              ylab("PI [rel. units]") + 
              theme(strip.text = element_text(hjust = -0.05), strip.background = element_blank(), aspect.ratio=4/NofPeriods)
  }
...
```{r BoxWithNotch, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = len, fig.width = 6}
for(x in 1:NofGroups)
  {
  PIprofile <- grouped.PIprofiles[[x]]
  PIprofile <- PIprofile[colSums(!is.na(PIprofile)) > 0] #remove empty columns
    PIplots[[x]] <- ggplot(melt(PIprofile), aes(,, value)) +
              geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
...
```{r BoxNoNotch, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.height = len, fig.width = 6, fig.align = "center"}
for(x in 1:NofGroups)
  {
  PIprofile <- grouped.PIprofiles[[x]]
  PIprofile <- PIprofile[colSums(!is.na(PIprofile)) > 0] #remove empty columns
...
```{r Violin, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = len, fig.width = 8}
...
if(NofGroups == 1){
stat = as.logical("FALSE")
} else {
stat = as.logical("TRUE")
}
```
