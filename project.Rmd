...
for(x in 1:NofGroups)
  ...
  PIcombined = tidyr::gather(key = "variable", value = "value", PIcombined) #melt data frames
  PIcombined["data"] = melt(PIprofile)$value
    PIplots[[x]] <- ggplot(melt(PIprofile), aes(variable, value)) +
              ...
              geom_jitter(data = PIcombined, aes(variable, data, fill=value), position=position_jitter(0.3), shape=21, size=3) +
              ggtitle(paste(project.data[["resources"]][[x]][["title"]], ", N=",nrow(PIprofile))) +
