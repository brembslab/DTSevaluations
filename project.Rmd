...
```{r altFlyHistogram, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 12, fig.width = 8, comment=NA, results='asis'}    
  ...
  {
    par(mfrow=c(6,3))
    par(mfrow=c(round(NofPeriods/5)*2, round(NofPeriods/5))) # sets the dimension of the multiplot, row x column
...
```{r Mean dwelling, eval=Dwell, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 10, fig.width = 4, comment=NA, results='asis'}
...
for(x in 1:NofGroups){
  ...
  colnames(dwellmeans[[x]])=c("period","value","Outcome","flyname")
  ...
  geom_hline(yintercept = 0, colour = "#887000", size = 1.2)+
  geom_jitter_interactive(tooltip = dwellmeans[[x]]$flyname, data_id = dwellmeans[[x]]$flyname , col = "grey", pch=21, size=1.3, width = 0.25)+
...
girafe( ggobj = plot_grid(plotlist=dwellmeanplots, ncol = 1), width_svg = 3.5+(NofPeriods), height_svg = 4*NofGroups)
```
