...
supHistos$group <- factor(supHistos$group, levels=paste(groupnames)) # sets the order/level of the data accordingly to groupnames
if('yt' %in% sequence$type || 'sw' %in% sequence$type)                  #for yt or sw experiments, plot fly data
      {
       ...
       print(ggplot(supHistos, aes(x = fly)) + 
            ...
            scale_fill_manual(labels = paste(groupnames,", n = ", samplesizes, sep=" "), values = c("#E7B800", "#00AFBB")) +
            theme_light() +
...
plotOMtracesMean(grouped.OMdataAfter) #call function to plot OM traces at start of experiment averaged over all flies
```
...
plotOMparams$group <- factor(plotOMparams$group, levels=paste(groupnames)) # sets the order/level of the data accordingly to groupnames
...
if(NofGroups==1)
  {
        for (v in 1:3) {
           ...
           print(ggplot(plotOMparams, aes(group, plotOMparams[[OMvariables[v]]])) +
              ...
              samplesizes.annotate(boxes, as.numeric(table(plotOMparams$group))))
        }
}
...
plotOMparamsBefore$group <- factor(plotOMparamsBefore$group, levels=paste(groupnames)) # set the level of the data accordingly to groupnames
OMvariables = head(names(plotOMparamsBefore),-2)
OMtitles = c("Optomotor Magnitude", "Rate Constant", "Asymmetry Index")
...
if(NofGroups==1)
  {
        for (v in 1:3) {
           ...
           print(ggplot(plotOMparamsBefore, aes(group, plotOMparamsBefore[[OMvariables[v]]])) +
              ...
              theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black", fill=NA)) +
              theme(axis.text.y = element_text(size=18))+ ylab(paste(OMtitles[v], " [rel. units]", sep = ""))+ xlab("Groups")+ theme(aspect.ratio=3/NofGroups)+
              samplesizes.annotate(boxes, as.numeric(table(plotOMparamsBefore$group))))
        }
}
...
plotOMparamsAfter$group <- factor(plotOMparamsAfter$group, levels=paste(groupnames)) # sets the order/level of the data
...
if(NofGroups==1)
  {
        for (v in 1:3) {
           ...
           print(ggplot(plotOMparamsAfter, aes(group, plotOMparamsAfter[[OMvariables[v]]])) +
              ...
              theme_light(base_size = 16) + theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black", fill=NA)) +
              theme(axis.text.y = element_text(size=18))+ ylab(paste(OMtitles[v], " [rel. units]", sep = ""))+ xlab("Groups")+ theme(aspect.ratio=3/NofGroups)+
              samplesizes.annotate(boxes, as.numeric(table(plotOMparamsAfter$group))))
        }
}
...
spectemp$group <- factor(spectemp$group, levels=paste(groupnames)) # sets the order/level of the data accordingly to groupnames
...
PIstatCombined$group <- factor(PIstatCombined$group, levels=paste(groupnames)) # sets the order/level of the data
  wilcoxon<-numeric()  
...
# plot PI box plot test against zero
  plots.singles<-list(ggplot(PIstatCombined, aes(group, PIs)) +
    ...
    samplesizes.annotate(boxes, as.numeric(table(PIstatCombined$group))) +
...
for (x in 1:NofGroups){
  PItype[[x]] = melt(PIstat[[x]], value.name = "PIs")
  PItype[[x]]$desc = project.data$resources[[x]]$description
  PItype[[x]]$group = project.data$resources[[x]]$name
  PItype[[x]]$category = unlist(CatStat[x])
  PItype[[x]] = na.omit(PItype[[x]])
  PIstatCombined = ldply(PItype, data.frame)
}
OMvariables = "PIs"
