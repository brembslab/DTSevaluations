...
for(x in 1:NofGroups)
  ...
  PIprofile <- grouped.PIprofiles[[x]]
  PIprofile <- PIprofile[colSums(!is.na(PIprofile)) > 0] #remove empty columns
  PIcolor <- grouped.PIcolor[[x]]
  PIcolor <- PIcolor[colSums(!is.na(PIcolor)) > 0]#remove empty columns
  PIcombined <- grouped.PIcombined[[x]]
  PIcombined <- PIcombined[colSums(!is.na(PIcombined)) > 0]

  PIcolor["new"] <- NA #the PIcolor does not melt. Adding an empty column fixes this problem
  PIcombined["new"] <- NA
  PIcombined = melt(PIcombined,id.vars="new") #melt data frames 
  PIcolor = melt(PIcolor,id.vars="new")
  n = length(unique(PIcombined$value))
    PIplots[[x]] <- ggplot(melt(PIprofile), aes(variable, value)) +
              ...
              geom_jitter(data = melt(PIprofile), aes(x=variable, y=value, fill=PIcombined$value), position=position_jitter(0.3), shape=21, size=3, colour="black") +
              ggtitle(paste(project.data[["resources"]][[x]][["title"]], ", N=",nrow(PIprofile))) +
              ...
              theme(aspect.ratio=aspect_ratio) +
              scale_fill_manual(values=wes_palette(n = n, name="FantasticFox1", type = c("discrete"))) + #changes the fill color using a Wes Anderson package. Because Wes Anderson.
