...
```{r Utest, eval=(threegroupstats & NofGroups==3), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
cat("\n## Statistical tests between three groups\n")

    utest1 = signif(wilcox.test(PIstat[[3]],PIstat[[1]])$p.value, 3)
    utest2 = signif(wilcox.test(PIstat[[3]],PIstat[[2]])$p.value, 3)#compare the two groups with a U-test and collect p-value
    w.statistic1 = signif(wilcox.test(PIstat[[3]],PIstat[[1]])$statistic, 3)
    w.statistic2 = signif(wilcox.test(PIstat[[3]],PIstat[[2]])$statistic, 3)
    #compute effect size Cohen's D
    cohend1 = signif(cohen.d(na.omit(PIstat[,3]), na.omit(PIstat[,1]))$estimate, 3)
    cohend2 = signif(cohen.d(na.omit(PIstat[,3]), na.omit(PIstat[,2]))$estimate, 3)
    #calculate statistical power
    ...
    #calculate Bayes Factor
    bayesF1=extractBF(ttestBF(na.omit(PIstat[[3]]), na.omit(PIstat[[1]])))
    bayesF2=extractBF(ttestBF(na.omit(PIstat[[1]]), na.omit(PIstat[[2]])))
    #calculate FPR for priors set in project file#
    #run first prior  
      prior=priorval[1]
    ...
    #run second prior
      ...
      fpz4=out2[1]
    #Power and likelihood ratio: NB for two sided test, need 2*y0
      LR1=out1[5]/(2*out[3])        #lik ratio (Hi1/H0) =y1/2*y0
      LR2=out2[5]/(2*out[3])
      #make tidy table of results
    results.utest<-(cbind(TestxCont1=c(signif[1],
                       ...
                       TestxCont2=c(signif[1],
                                       ...
                                       signif(fpz4, 3),
                                       signif(LR2, 3))))
     #results.utest=cbind(results.utest1,results.utest2)
