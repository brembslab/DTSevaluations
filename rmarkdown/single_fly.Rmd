...
```{r dytracesdown, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 4, fig.width = 12}    
    ...
    threshold <- 80 ##Sets the cutoff value for low variance
    pot_pause <- var_vector < threshold ##Makes a logical vector
    ##prob_pause[x] <- pot_pause[x] && (pot_pause[x-1] | pot_pause[x+1]) ##Includes only stretches of low variance,     still workshopping
    num_pause <- sum(pot_pause, na.rm=TRUE) ##Determines number of TRUE values 
    per_pause <- num_pause/length(pot_pause)*100 ##Calculates the ratio of time in pause
