...
direc_vec <- c("left", "right", "center")
def_pause_list <- vector("list", length(direc_vec))
names(def_pause_list) <- direc_vec
for (x in direc_vec) {
  var_vec <- frollvar(rawdata$fly, n = span, align = x, fill = NA) ##Takes a rolling variance
  ...
  def_pause_temp$values[is.na(def_pause_temp$values)] <- FALSE
  ...
  def_pause_list[[x]] <-def_pause_temp
}
