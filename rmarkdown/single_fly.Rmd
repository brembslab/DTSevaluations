...
threshold <- 50 ##Sets threshold for low variance
min_length <- 30 ##Sets the minimum length considered a flight pause
direc_vec <- c("left", "right", "center") #number of alignments
def_pause_list <- vector("list", length(direc_vec)) #Creates a list
names(def_pause_list) <- direc_vec #Assigns names to list
for (i in direc_vec) {
  var_vec <- frollvar(rawdata$fly, n = span, align = i, fill = NA) ##Takes a rolling variance
  pot_pause <- var_vec < threshold ##Finds variance below threshold
  ...
  def_pause_list[[i]] <-def_pause_temp #Puts output in a list
}
...
new_format_data <- xts::xts(as.numeric(rawdata$fly), order.by = as.POSIXct(rawdata$time / 1000, origin = "1970-01-01")) ##Formats     data for dygraphs
...
rm(graph_shaded) ##deletes old dygraph
gc()
