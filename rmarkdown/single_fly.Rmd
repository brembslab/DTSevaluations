...
```{r dytracesdown, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 4, fig.width = 12}    
    ...
    span <- 11 ##Sets the span for the rolling variance
    var_vector <- frollvar(rawdata$fly, n = span, align = "center") ##Takes a rolling variance
    ...
    ratio <- length(low)/20 ##Finds bottom 5% of variance
    low <- low[1:ratio]
    ...
    min_length <- 40 ##Sets the minimum length considered a flight pause
    def_pause <- rle(pot_pause) ##Analyzes run length of TRUE on pot_pause
    too_short <- def_pause$values & def_pause$lengths < min_length
    def_pause$values[too_short] <- FALSE
