...
ratiovel <- length(lowvel)/5 ##Finds bottom 20% of velocity
lowvel <- lowvel[1:ratio]
cutoff <- mean(lowvel, na.rm=TRUE)
lowvel <- vel < cutoff
velvar <- frollvar(abvel, n = span, align = "right", fill = NA) ##Takes a rolling variance
span <- 21 ##Sets the span for the rolling variance
threshold <- 200 ##Sets threshold for low variance
min_length <- 20 ##Sets the minimum length considered a flight pause
...
for (y in direc_vec) {
  ...
  ratio <- length(low)/5 ##Finds bottom 10% of variance
  low <- low[1:ratio]
