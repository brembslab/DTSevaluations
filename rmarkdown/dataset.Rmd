...
<center><h2>for time series data set: "<b>`r paste(dataset.data$experiment$title)`</b>"</h2></center>
...
if(NofGroups==3 & length(unique(groupdescriptions))==2){
  OMparamsSingleton <- grouped.OMparams[[grep(singleton[1], grouped.OMparams)]]
  OMparamsDoubleton1 <- grouped.OMparams[[grep(doubleton[1], grouped.OMparams)[1]]]
  OMparamsDoubleton2 <- grouped.OMparams[[grep(doubleton[1], grouped.OMparams)[2]]]
  ...
  OMparams_1 <- grouped.OMparams[[1]]
  OMparams_2 <- grouped.OMparams[[2]]
  OMparams_3 <- grouped.OMparams[[3]]
  gname<-t(as.data.frame(groupnames))
...
if(NofGroups==3 & length(unique(groupdescriptions))==2) {
  OMparamsBeforeSingleton <- grouped.OMparamsBefore[[grep(singleton[1], grouped.OMparamsBefore)]]
  OMparamsBeforeDoubleton1 <- grouped.OMparamsBefore[[grep(doubleton[1], grouped.OMparamsBefore)[1]]]
  OMparamsBeforeDoubleton2 <- grouped.OMparamsBefore[[grep(doubleton[1], grouped.OMparamsBefore)[2]]]
  ...
  OMparamsBefore_1 <- grouped.OMparamsBefore[[1]]
  OMparamsBefore_2 <- grouped.OMparamsBefore[[2]]
  OMparamsBefore_3 <- grouped.OMparamsBefore[[3]]
...
#for three groups with statistics
if(NofGroups==3 & length(unique(groupdescriptions))==2){
  OMparamsAfterSingleton <- grouped.OMparamsAfter[[grep(singleton[1], grouped.OMparamsAfter)]]
  OMparamsAfterDoubleton1 <- grouped.OMparamsAfter[[grep(doubleton[1], grouped.OMparamsAfter)[1]]]
  OMparamsAfterDoubleton2 <- grouped.OMparamsAfter[[grep(doubleton[1], grouped.OMparamsAfter)[2]]]
  ...
  OMparamsAfter_1 <- grouped.OMparamsAfter[[1]]
  OMparamsAfter_2 <- grouped.OMparamsAfter[[2]]
  OMparamsAfter_3 <- grouped.OMparamsAfter[[3]]
  PIname<-c(unique(OMparamsBefore_1$group), unique(OMparamsBefore_2$group),unique(OMparamsBefore_3$group))
  for (v in 1:4) {
    cat(paste("\n\n## ", OMtitles[v], " at end of experiment\n\n", sep = ""))
...
```{r Utest, eval=(PIs_present & twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 10, comment=NA, results='asis'}
...
if(NofGroups == 3 & length(unique(groupdescriptions))==2) {
   PItypeSingleton <- PItype[[grep(singleton[1], PItype)]]
   PItypeDoubleton1 <- PItype[[grep(doubleton[1], PItype)[1]]]
   PItypeDoubleton2 <- PItype[[grep(doubleton[1], PItype)[2]]]
...
```{r Utestpooled, eval=(PIs_present & twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 12, comment=NA, results='asis'}
