...
Count=1
  ...
  for (t in 1:NofGroups) {
    if(any(grepl("test", tolower(grouped.OMparams[[t]]$desc))) | any(grepl("exp", tolower(grouped.OMparams[[t]]$desc))) | any(grepl("experimental", tolower(grouped.OMparams[[t]]$desc)))==TRUE){
  ...
  for (v in 1:4) {
    ...
    results.utest<-data.frame(cbind(TestxCont1=c(signif[1],
                                    ...
                                    TestxCont2=c(signif[1],
                                                 ...
                                                 signif(fpz4, 3),
                                                 signif(LR2, 3))))
    colnames(results.utest)<-c(paste(PIname[1], PIname[2], sep="\n"),paste(PIname[1], PIname[3], sep="\n"))  
...
plotOMparams$group <- factor(plotOMparams$group, levels=unique(plotOMparams$group))
      ...
      geom_signif(comparisons = list(c(gname[,1], gname[,2]), c(gname[,2], gname[,3])),
                map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                textsize=5, vjust=0.1) +
      geom_signif(comparisons = list(c(gname[,1], gname[,3])),
                map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
...
if(NofGroups==3)
{
  ...
  Exp=list()
  for (t in 1:NofGroups) {
    if(any(grepl("test", tolower(grouped.OMparamsBefore[[t]]$desc))) | any(grepl("exp", tolower(grouped.OMparamsBefore[[t]]$desc))) | any(grepl("experimental", tolower(grouped.OMparamsBefore[[t]]$desc)))==TRUE){
...
if(NofGroups==3){
  ...
  Exp=list()
  for (t in 1:NofGroups) {
    if(any(grepl("test", tolower(grouped.OMparamsAfter[[t]]$desc))) | any(grepl("exp", tolower(grouped.OMparamsAfter[[t]]$desc))) | any(grepl("experimental", tolower(grouped.OMparamsAfter[[t]]$desc)))==TRUE){
...
```{r WilcoxonRaincloud, eval=wil, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.height = 7, fig.width = 6, comment=NA, results='asis'}
...
#create variable for plotting    
    HeatOn=as.integer(levels(as.factor(PIstatCombined$category)))
    
# plot PI box plot test against zero
  plots.singles[[2]]<-ggplot(PIstatCombined, aes(group, PIs, fill=group, color=group)) +
    geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
    geom_rain(cov = "category",
              point.args = list(size = 6, shape=PIstatCombined$category),
              point.args.pos = list(position = position_jitter(width = 0.09)),
              boxplot.args = list(fill="white", color = "black", outlier.shape = NA, size=0.9),
              boxplot.args.pos = list(position = ggpp::position_dodgenudge(x = .22), width = 0.15),
              violin.args = list(color = NA, alpha = .8),
              violin.args.pos = list(side = "r", width = 0.7, position = position_nudge(x = 0.22))) +
    stat_summary(fun.y="mean", color="grey", shape="—", size = 2.5, fill=boxcolors, position = position_nudge(0.22))+
    stat_summary(fun.y="median", color="black", shape="—", size = 2.5, fill=boxcolors, position = position_nudge(0.22))+
    ...
    guides(fill = 'none', color = guide_legend(title = "Heat on:"))+ # remove legend for groups, add legend title for categories
    theme_light(base_size = 16) + 
...
```{r Utest, eval=(PIs_present & twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 10, fig.width = 5, comment=NA, results='asis'}
...
```{r estimationstats, eval=(PIs_present & twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
...
```{r splitviolins, eval=(PIs_present & twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
cat("\n## Split Violin Plot\n")
...
```{r raincloudplots, eval=(PIs_present & twogroupstats & NofGroups==2), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
...
```{r UtestG3, eval=(PIs_present & threegroupstats & NofGroups==3), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 10, fig.width = 6, comment=NA, results='asis'}
...
Exp=list()
for (t in 1:NofGroups) {
  if(any(grepl("test", tolower(PItype[[t]]$desc))) | any(grepl("exp", tolower(PItype[[t]]$desc))) | any(grepl("experimental", tolower(PItype[[t]]$desc)))==TRUE){
...
```{r estimationstatspooled, eval=(PIs_present & twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
 ...
 unpaired_median_diff <- dabest(PIstatPooled, group, PIs, idx = groupnames, paired = FALSE) %>% median_diff() #create estimation results list
 print(plot(unpaired_median_diff)) #plot results
