...
```{r URIs_check, echo=FALSE, results='asis'}
if (URIs_match) {
  cat("**Recording:** [", paste(singleflydata$URIs$recording), "](https://doi.org/", paste(singleflydata$URIs$recording), ") ",
      "**Analysis:** [", paste(singleflydata$URIs$analysis), "](https://doi.org/", paste(singleflydata$URIs$analysis), ") ",
      "**Datamodel: ** [", paste(singleflydata$URIs$datamodel), "](https://doi.org/", paste(singleflydata$URIs$datamodel), ") ",
      sep="")
} else {
  cat("WARNING: URIs in YAML and XML files do not match! Please check html files created for individual flies to access the Recording and Datamodel identifiers. ", "**Analysis:** [", paste(yaml_URIs$analysis), "](https://doi.org/", paste(singleflydata$URIs$analysis), ") ")
}
```
...
for(x in 1:NofGroups)
  ...
  for (i in 1:NofPeriods){
      ...
      hist(x = df$fly, breaks = 4000,xlim = maxflymax, xlab="", border= sequence$histocolor[i],  col = sequence$histocolor[i], main=paste("Period ", i)) #plot histogram
    ...
    hist(grouphistdata[[x]]$fly, breaks = 300,xlim = maxflymax, xlab=paste(FlyBehavior, "[arb units]"), main=paste("Pooled Behavior Histogram", groupnames[[x]]))
  }  
...
plotOMtracesSEM(grouped.OMdata_swapped)
```
...
plotOMtracesSEM(grouped.OMdataBefore_swapped)
...
plotOMtracesSEM(grouped.OMdataAfter_swapped)
```
...
if(NofGroups==3)
{   
  if(length(unique(groupdescriptions))==2){
...
```{r OMPI_correlation, eval=(PIs_present & beforeafter & any(sequence$type=="yt") & NofGroups==2), echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.height = 14, fig.width = 14, comment=NA, results='asis'}
...
CorrPlots[[1]] <- ggplot(OMPIcorr, aes(pretestPIs.category, plotOMparamsBefore.AI.OM.)) +           #plot the correlation between PIs and OM asymmetry before training
              ...
              stat_poly_line() +
              stat_poly_eq(label.y=1, use_label(c("eq", "adj.R2", "f", "p", "n")), size=6) +
...
mOMPIbefore=subset(reshape2::melt(OMPIcorr), variable %in% c("plotOMparamsBefore.AI.OM.", "pretestPIs.category")) # melt dataframes with values before training for raincloudplots
...
CorrPlots[[2]] <- ggplot(mOMPIbefore, aes(variable, value, fill=variable, color=variable)) +
              ...
              scale_y_continuous(breaks = seq(-1, 1, .2), limits = c(-1,1)) +
              scale_fill_manual(values = colorrange[3:4]) + #color of the violin plot
              scale_color_manual(values = colorrange[3:4]) + #color of the points
              ...
              theme(panel.grid.minor = element_blank(), 
                    ...
                    panel.border = element_rect(size = 0.5, linetype = "solid", colour = "black", fill=NA)) +
              theme(axis.text.x = element_text(size=15), axis.text.y = element_text(size=15)) + ylab(paste("Indices [rel. units]"))+
...
mOMPIafter=subset(reshape2::melt(OMPIcorr), variable %in% c("plotOMparamsAfter.AI.OM.", "postPIs.category"))  # melt dataframes with values after training for rainclodplots
...
plotOMparamsBefore[plotOMparamsBefore$group=="right",]$AI.OM.=-plotOMparamsBefore[plotOMparamsBefore$group=="right",]$AI.OM.
plotOMparamsAfter[plotOMparamsBefore$group=="right",]$AI.OM.=-plotOMparamsAfter[plotOMparamsBefore$group=="right",]$AI.OM.
} #ends the if clause that checks for one of the groups being clalled "right". Needs to be fixed when this section is generalized to work with any number and design of groups.
```
...
```{r WilcoxonRaincloud, eval=wil, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.height = 8, comment=NA, results='asis'}
...
#add table with results and plot
  ...
  grid.arrange(grobs = plots.2test, ncol=2, nrow=1)
...
# plot two PIs with asterisks
  plots.2test<-list(ggplot(PIstatCombined, aes(group, PIs, fill=group, color=group)) +
    geom_hline(yintercept = 0, colour = "#887000", size = 1.2) +
    {if(typeof(HeatOn)=="double") #if the shapes are given as numbers (default), give them a black outline
    geom_rain(point.args = list(size = 5, shape=HeatOn, color = "black", alpha=as.integer(as.factor(PIstatCombined$category))/max(as.integer(as.factor(PIstatCombined$category)))),
              point.args.pos = list(position = ggpp::position_jitternudge(width = 0.1, x = -0.22, nudge.from = "jittered")),
...
estimation_stats <- dabestr::load(PIstatCombined, x = group, y = PIs, idx = groupnames) #load PIs into dabestr format
if(table(PIstatCombined$group)[1]==table(PIstatCombined$group)[2]) {   #make sure both groups have identical N
  est_meanDiff=mean_diff(estimation_stats)       #calc mean differences
  est_medianDiff=median_diff(estimation_stats)   #calc median differences
  est_cohensD=cohens_d(estimation_stats)         #calc Cohen's DD
  est_hedgesG=hedges_g(estimation_stats)         #calc Hedges' G
#plot all results
cat("\n### Mean Difference\n")
print(dabest_plot(est_meanDiff))
cat("\n")  #don't ask me why this is needed!
cat("\n### Median Difference\n")
print(dabest_plot(est_medianDiff))
cat("\n")
cat("\n### Cohen's D\n")
print(dabest_plot(est_cohensD))
cat("\n")
cat("\n### Hedges' G\n")
print(dabest_plot(est_hedgesG))
} else {
 cat("**No plot here because sample size needs to be equal in both groups**")
}
```
...
```{r UtestG3, eval=(PIs_present & threegroupstats & NofGroups==3), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 12, comment=NA, results='asis'}
...
PItype <- list()
for (x in 1:NofGroups){
  PItype[[x]] = data.frame(PIstat[[x]])                        #get PIs for each group
  colnames(PItype[[x]])[1] <- "value"                          #set the right name for the column with the PIs
  PItype[[x]]$desc = dataset.data$resources[[x]]$description   #add group description
  PItype[[x]]$name = dataset.data$resources[[x]]$name          #add group name
}
...
```{r Utestpooled, eval=(PIs_present & twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", comment=NA, results='asis'}
...
```{r estimationstatspooled, eval=(PIs_present & twogroupstats & PoolGrps), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
 ...
 estimation_stats <- dabestr::load(PIstatPooled, x = group, y = PIs, idx = dabest_groupnames) #load PIs into dabestr format
if(table(PIstatPooled$group)[1]==table(PIstatPooled$group)[2]) {   #make sure both groups have identical N
  est_meanDiff=mean_diff(estimation_stats)       #calc mean differences
  est_medianDiff=median_diff(estimation_stats)   #calc median differences
  est_cohensD=cohens_d(estimation_stats)         #calc Cohen's DD
  est_hedgesG=hedges_g(estimation_stats)         #calc Hedges' G
