...
```{r UtestG3, eval=(PIs_present & threegroupstats & NofGroups==3), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 7, fig.width = 13, comment=NA, results='asis'}
...
plots.2test<-ggplot(PIstatCombined, aes(group, PIs)) +
                    ...
                    geom_signif(comparisons = list(c(colnames(PIstat[1]), colnames(PIstat[2]))
                                                   , c(colnames(PIstat[2]), colnames(PIstat[3]))), 
                                map_signif_level= c("***"= signif[3],"**"= signif[2], "*"= signif[1]),
                                textsize=5, vjust=0.1) +
