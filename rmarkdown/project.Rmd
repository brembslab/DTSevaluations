...
#for one group without statistics
if(NofGroups==1 | (NofGroups >= 3 & !threegroupstats))
  {
        for (v in 1:4) {
...
if(NofGroups==3 & threegroupstats)
{
  Count=1
  Control=list()
  Exp=list()
  for (t in 1:NofGroups) {
    if(any(grepl("test", tolower(grouped.OMparamsBefore[[t]]$desc)) | grepl("exp", tolower(grouped.OMparamsBefore[[t]]$desc)) | grepl("experimental", tolower(grouped.OMparamsBefore[[t]]$desc)))==TRUE){
...
if(NofGroups==3 & threegroupstats){
  ...
  Exp=list()
  for (t in 1:NofGroups) {
    if(any(grepl("test", tolower(grouped.OMparamsAfter[[t]]$desc)) | grepl("exp", tolower(grouped.OMparamsAfter[[t]]$desc)) |      grepl("experimental", tolower(grouped.OMparamsAfter[[t]]$desc)))==TRUE){
...
```{r Wilcoxon, eval=wil, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.align = "center", fig.height = 6, fig.width = 10, comment=NA, results='asis'}
...
Exp=list()
for (t in 1:NofGroups) {
  if(any(grepl("test", tolower(PItype[[t]]$desc)) | grepl("exp", tolower(PItype[[t]]$desc)) | grepl("experimental", tolower(PItype[[t]]$desc)))==TRUE){
