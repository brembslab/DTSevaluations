...
# Dwelling time distribution (punished)

```{r dwelling, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 12, fig.width = 8}
...
# dwellpos = rawdata[rawdata["fly"] >= 0, ] # sort negative data
...
# sum = sum(dwellpos$value) #total number of data points
# dwellpos$perctest = 100-(cumsum(dwellpos$value/sum)*100) # subtracts the cumulative
# dwellpos <- na.omit(transform(dwellpos, perctest = c(NA, perctest[-nrow(dwellpos)])))
# dwellpos$Var1 = (dwellpos$Var1 * 50)/1000
# insert <- c(0,100,100)
# dwellpos = rbind(dwellpos, insert)
# ggplot(data=dwellpos, aes(x=Var1, y=perctest)) +
#   geom_line(color="red")+
#   geom_point()+
#   scale_y_log10( breaks=c(0,10,100))+
#   scale_x_log10() +
#   ggtitle(paste("Dwelling time distributions (unpunished)", flyname))+
#   theme_light() +
#   labs(x="time [sec]", y="% events") +
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
#   theme(plot.title = element_text(color="black", size=12, face="bold", hjust = 0.5))
