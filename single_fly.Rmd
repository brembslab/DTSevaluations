...
# Find the midpoint in left and right turning in averaged traces (plotOM)
OMmidpoint = plotOM$time[nrow(plotOM)]/2

#right turning traces
x=plotOM$time[plotOM$time<=OMmidpoint]                       #time
y=plotOM$means[plotOM$time<=OMmidpoint]+abs(plotOM$means[1]) #mean right OM response normalized to start from zero
m<-nls(y~a*x/(b+x))                                          #use Michaelis-Menten to fit the data
right=coef(m)                                                #put the right turning coefficients in a vector

#left turning traces
x=plotOM$time[plotOM$time>OMmidpoint]                        #time
x=x-x[1]                                                     #set time to start from zero
y=-plotOM$means[plotOM$time>OMmidpoint]                      #mean left OM response
y=y+abs(y[1])                                                #normalized to start from zero
m<-nls(y~a*x/(b+x))                                          #use Michaelis-Menten to fit the data
left=coef(m)                                                 #put the left turning coefficients in a vector

#calculate averages and asymmetries
aveOMcoeff=(right+left)/2                                    #mean asymptote and halfmaximal time
aveOMcoeff[2]=aveOMcoeff[2]/1000                             #halfmaximal time in seconds
...
names(tempOMparams) = c("asymptote", "halfmax_time","AI")    #column names
rownames(tempOMparams)[1]=as.character(flyname)              #row names
kable(tempOMparams)                                          #plot table with data

#export coefficients in dataframwe
if(!exists("OMparams"))
{
