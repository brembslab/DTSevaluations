...
dwellmeans$punished = data.frame(value=rep(2, NofPeriods), punishment=rep("punished",NofPeriods), period = sprintf("PI%d", 1:NofPeriods)) #create an empty list at the same length as number of periods, with a set of three prenamed columns
dwellmeans$unpunished = data.frame(value=rep(2, NofPeriods), punishment=rep("unpunished",NofPeriods), period = sprintf("PI%d", 1:NofPeriods)) #create an empty list at the
for(i in 1:NofPeriods){
  ...
  dwelltimes$punished[[i]] = sort(unique(dwelldata$punished[[i]]))
  dwelltimes$unpunished[[i]] = sort(unique(dwelldata$unpunished[[i]]))

 
  dwellmeans$unpunished$value[[i]] = mean(dwelldata$unpunished[[i]])
  dwellmeans$punished$value[[i]] = mean(dwelldata$punished[[i]])
