...
#add training/test to the graphs
  ...
  ###divide raw data into period chunks and prepare period data for later use in group evaluations
      ...
      ##add column that indicates if the fly was punished or not
      if(sequence$contingency[i] == 'left_torque'){rawdata$punished <- ifelse(rawdata$fly < 0, 1, 0)              #negative torque becomes marked 1 if punished
      }else if(sequence$contingency[i] == 'right_torque') {rawdata$punished <- ifelse(rawdata$fly >= 0, 1, 0)     #positive torque becomes marked 1 if punished
      }else if(sequence$contingency[i] == '2_4_Q'){rawdata$punished <- ifelse(abs(rawdata$a_pos) >= 450 & abs(rawdata$a_pos) <= 1350, 1, 0)#mark 2_4_Q data with 1 if punished
      }else if(sequence$contingency[i] == '1_3_Q'){rawdata$punished <- ifelse(abs(rawdata$a_pos) < 450 & abs(rawdata$a_pos) > 1350, 1, 0)} #mark 1_3_Q with 1 if punished
