...
```{r dwellingtime, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 12, fig.width = 8}
y = c("punished", "unpunished")
for(i in 1:NofPeriods){
  temp  <- rawdata[rawdata$period == i, ]
  temp <- temp[c("time","fly")]
  if(!exists("dwell")){dwell <- NA}
  if(!exists("scattertotal")){scattertotal <- list()}
  dwell$punished = temp[temp["fly"] < 0, ] # sort negative data
  dwell$unpunished = temp[temp["fly"] >= 0, ] # sort positive data
  scatter = dwell
  for(t in 1:length(y)){
    scatterperiod[[t]] <- reshape2::melt(lengths(split(scatter[[t]][[1]], cumsum(c(TRUE, diff(scatter[[t]][[1]]) != 50))))) #finds cumulative data 
    scatterperiod[[t]]$period = i
    scatterperiod[[t]]$punishment = y[[t]]
    scatterperiod[[t]][[1]] <- (scatterperiod[[t]][[1]])*0.05
    scattertemp = do.call(rbind, scatterperiod)
    ...
    dwell[[t]] <- transform(dwell[[t]], value = c(100, value[-nrow(dwell[[t]])])) #shift data down
    dwelltemp <- ldply (dwell, data.frame)
  }
  scattertotal[[i]] = scattertemp
  dwelltotal[[i]] = dwelltemp
  dwellingtime[[i]] = ggplot() +
...
scattertemp = do.call(rbind, scattertotal)
dwellingtime[[NofPeriods+1]] <- ggplot(scattertemp, aes(x = factor(period), y = value, fill = punishment)) +
  geom_boxplot() +
  scale_y_log10( breaks=c(0,10,100))
