...
for(i in 1:NofPeriods){
  ...
  #the following monstrosity adds a specific color to any of the six possible contingency/pattern combinations   
  }
  if(sequence$pattern[i]== 7 & sequence$contingency[i]=="left_torque"){ 
    sequence$PIcolor[i]="red"
  }
  if(sequence$pattern[i]== 7 & sequence$contingency[i]=="right_torque"){
    sequence$PIcolor[i]="blue"
  }
  if(sequence$pattern[i]== 8 & sequence$contingency[i]=="left_torque"){
    sequence$PIcolor[i]="green"
  }
  if(sequence$pattern[i]== 7 & sequence$contingency[i]=="right_torque"){
    sequence$PIcolor[i]="orange"
  }
  if(sequence$pattern[i]== 0 & sequence$contingency[i]=="left_torque"){
    sequence$PIcolor[i]="black"
  }
  if(sequence$pattern[i]== 0 & sequence$contingency[i]=="right_torque"){
    sequence$PIcolor[i]="purple"
  }
  if(sequence$contingency[i]==""){
  sequence$PIcolor[i]=NA
  }
      ...
      if(!exists("PIcolor")){PIcolor <- data.frame(matrix(ncol = NofPeriods))}
      if(!exists("PIcombined")){PIcombined <- data.frame(matrix(ncol = NofPeriods))}
      ...
      } else if (sequence$type[i]=="yt" || sequence$type[i]=="sw_blue" || sequence$type[i]=="sw_green") #calculate torque PIs
      {
        ...
        if(l==1){PIprofile[1,i]=sequence$lambda[i]}
        if(l==1){PIcolor[1,i]=sequence$PIcolor[i]}
        sequence$combined[i] <- paste(sequence$pattern[i],sequence$contingency[i]) #combines pattern and contingency into a new column
        if(l==1){PIcombined[1,i]=sequence$combined[i]}
      }
