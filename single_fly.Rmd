...
#add columns for PIs to sequence data
    ...
    for(i in 1:NofPeriods){
     ...
     if(sequence$contingency[i]==""){
      ...
      #fly behavior
      min = c(-round_any(max(abs(flyrange)), -100, f=ceiling))
      max = c(round_any(max(abs(flyrange)), -100, f=ceiling))
      zero = 0
      cont <- any(grepl("left_torque", sequence$contingency[i])==TRUE)
        if(any(grepl("left_torque", sequence$contingency[i])==FALSE))
          {
          flyhistos[[i]] <- ggplot(data=temp, aes_string(temp$fly)) +
            ...
            scale_x_reverse(limits = c(-min, -max),
                                 breaks= c(-min, -max, by = 2),
                                 labels = c(min, max, 0), , expand = c(0.03, 0)) +
            labs(title = flyname, "Period", i,
                 subtitle = sequence$type[i])
          } else {
            flyhistos[[i]] <- ggplot(data=temp, aes_string(temp$fly)) +
              geom_histogram(binwidth = 3, fill = sequence$histocolor[i]) +
              labs(x=paste(FlyBehavior, "[arb units]"), y="frequency") +
