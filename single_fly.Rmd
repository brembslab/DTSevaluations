...
for(t in 1:length(punishment)){
  if(!exists("means")){means <- list()}
  means[[t]] = data.frame(value=rep(2, NofPeriods), punishment=rep(0,NofPeriods)) #create an empty list at the same length as number of periods, with a set of three prenamed columns

  means[[t]]$punishment <-  punishment[[t]] #add punished or unpunished to the punishment column
  means[[t]]$period = sprintf("PI%d", 1:NofPeriods) #add periods to the data
}
...
##Calculations for dwelling time## 
  for(t in 1:length(punishment)){
    ...
    scatterperiod[[t]][[1]] <- (scatterperiod[[t]][[1]])*0.05 #Transform each datapoint to seconds by multiplying with 0.05
     ...
     means[[t]]$value[[i]] = mean(scatterperiod[[t]]$value)
...
scatterplot = list()
#The function below is an inverse hyperbolic sine transformation to generate negative logarithmic data. I totally understand it and did not at all just copy it...
