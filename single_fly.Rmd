...
rightOMdataBefore<-filter(rawdata, rawdata$period %in% rightOMbefore) #extract only right turning arena periods
...
leftOMdataBefore<-filter(rawdata, rawdata$period %in% leftOMbefore)  #extract only left turning arena periods
...
### extract optomotor traces and generate dataframe for right (clockwise) turning arena
rightOMafter=which(grepl("optomotorR", tail(sequence$type, 8)))+NofPeriods-8 #collect the right turning OM periods in the last half of the experiment
rightOMdataAfter<-filter(rawdata, rawdata$period %in% rightOMafter) #extract only right turning arena periods
...
### extract optomotor traces and generate dataframe for left turning (counter-clockwise) arena
leftOMafter=which(grepl("optomotorL", tail(sequence$type, 8)))+NofPeriods-8 #collect the left turning OM periods in the last half of the experiment
leftOMdataAfter<-filter(rawdata, rawdata$period %in% leftOMafter)  #extract only left turning arena periods
...
leftOMdataAfter$time=((leftOMdataAfter$time)*50)+max(rightOMdataAfter$time) #make 20Hz data into ms time scale, starting after right turning data
