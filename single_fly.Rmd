...
temp$sign <- ifelse(temp$fly < 0, 0, 1) #If temp$fly is equal to 0 then it adds 0 in the sign column, everything else gets a 1. It automatically sets 0 as to be punished and unpunished is set to 1.
dwellperiod <- split(temp, temp$period) #splits data into periods

for(i in 1:NofPeriods){
  
  #set a vector to sort the punished/unpunished data
  dwellvector = 1 #initialize the dwellvector dataframe.
  ...
  if (length(unique(dwellperiod[[i]]$sign)) == 2) { 
    ...
    dwelldata$unpunished[[i]] <- reshape2::melt(table(dwellperiod[[i]][seq(dwellvector$Punished, length(dwellperiod[[i]]), 2)]*0.05)) 
    dwelldata$punished[[i]]<- reshape2::melt(table(dwellperiod[[i]][seq(dwellvector$Unpunished, length(dwellperiod[[i]]), 2)]*0.05)) 

    #subtracts the cumulative percentage
    dwelldata$unpunished[[i]]$value<- 100-(cumsum(dwelldata$unpunished[[i]]$value/(sum(dwelldata$unpunished[[i]]$value)))*100)
    dwelldata$punished[[i]]$value<- 100-(cumsum(dwelldata$punished[[i]]$value/(sum(dwelldata$punished[[i]]$value)))*100)
    
    #shift data down and replaces top value with 100%
    dwelldata$unpunished[[i]] <- transform(dwelldata$unpunished[[i]], value = c(100, value[-nrow(dwelldata$unpunished[[i]])]))
    dwelldata$punished[[i]] <- transform(dwelldata$punished[[i]], value = c(100, value[-nrow(dwelldata$punished[[i]])]))
    ...
    dwelldata$unpunished[[i]] = data.frame(Var1=rep(0), value=rep(0))
    dwelldata$punished[[i]] = data.frame(Var1=rep(120), value=rep(100))
    ...
    dwelldata$unpunished[[i]] = data.frame(Var1=rep(120), value=rep(100))
    dwelldata$punished[[i]] = data.frame(Var1=rep(0), value=rep(0))}

  #plots the data
  dwellingplot[[i]] = ggplot() +
    geom_line(data = dwelldata$punished[[i]], aes(x = Var1, y = value, color = "blue"))+
    geom_line(data = dwelldata$unpunished[[i]], aes(x = Var1, y = value, color = "red"))+
    geom_point(data = dwelldata$punished[[i]], aes(x = Var1, y = value, color = "blue"))+
    geom_point(data = dwelldata$unpunished[[i]], aes(x = Var1, y = value, color = "red"))+
    scale_y_log10( breaks=c(0,10,100))+
    scale_x_log10() +
    ggtitle(paste(flyname, "Period", i)) +
    theme_light() +
