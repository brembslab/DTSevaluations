...
#add columns for PIs to sequence data
    ...
    for(i in 1:NofPeriods){
     ...
     if(sequence$contingency[i]==""){
      ...
      ##only look at period data and save period data for later use
      temp  <- rawdata[rawdata$period == i, ]
      keeps = c("a_pos","fly")
      if((sequence$type[i]=="fs"||sequence$type[i]=="color") & sequence$contingency[i]=="2_4_Q")
      {
        temp_shifted = temp
      ...
      } else period.data[[i]] <- temp[keeps] #list only arena position and fly behavior data by period
...
ggplot(data=OMdata, aes(x=OMdata$time/1000, y=OMdata[[as.character(flyname)]])) + 
  geom_rect(aes(xmin = mean(OMdata$time/1000),xmax = Inf,ymin = -Inf, ymax = Inf),fill=("lightgrey"))+
