...
for(i in 1:NofPeriods){
  ...
  ###divide raw data into period chunks and prepare period data for later use in group evaluations
      ...
      #if(!exists("PInorm")){PInorm <- data.frame)}
      ...
      sequence$norm[i] = sequence$lambda[i]
  ...
  pretestfly[n] = sequence$lambda[n]
  if (sequence$outcome[n] == 1)
    break;
  
  n = n+1}
pretest = mean(pretestfly)
sequence$norm = sequence$lambda - pretest
    }
...
# Performance Index {.tabset .tabset-fade}
...
## Preference Indices Normalized

```{r PIsnorm, eval=(!all(is.na(sequence$lambda))), echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.align = "center", fig.height = 6, fig.width = 8, comment=NA, results='asis'}
      barplot(as.vector(na.omit(sequence$norm)), main = paste("Performance Indices (normalized)", flyname),
            xlab="Time [periods]",
            ylab="PI [rel. units]",
            space=0,
            col = as.vector(na.omit(sequence$color)))
