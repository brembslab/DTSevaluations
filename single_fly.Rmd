...
sequence$normalized = sequence$lambda-pretest
n=1
...
max = round(max(sequence$lambda))

      barplot(as.vector(na.omit(sequence$lambda)), main = paste("Performance Indices (not normalized)", flyname),
            xlab="Time [periods]",
            ylab="PI [rel. units]",
            space=0,
            ylim=c(0,max),
...
```{r, echo=FALSE}
      barplot(as.vector(na.omit(sequence$normalized)), main = paste("Performance Indices (normalized", flyname),
            xlab="Time [periods]",
            ylab="PI [rel. units]",
            space=0,
            ylim=c(0,max),
