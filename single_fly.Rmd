...
for (i in 1:NofPeriods){
  df = subset(subset, subset$period == i)
  hist(x = df$fly, breaks = 100,xlim = round(range(subset$fly))*1.1, xlab=paste(FlyBehavior, "[arb units]"), border= sequence$histocolor[i],  col = sequence$histocolor[i], main=paste("Period ", i))
  abline(v = 0, col="black", lwd=2, lty=2)
 
  }
hist(subset$fly, breaks = 100, main=paste(flyname, "Total"))
```
